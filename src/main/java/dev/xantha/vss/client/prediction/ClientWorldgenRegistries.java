...
final class ClientWorldgenRegistries {
    ...
    private ClientWorldgenRegistries(RegistryAccess fallback) {
        ...
        this.ops = RegistryOps.create(JsonOps.INSTANCE, (RegistryOps.RegistryInfoLookup) this::lookup);
    }

    static ClientWorldgenRegistries decode(JsonObject root, RegistryAccess fallback) {
        return decode(root, fallback, net.minecraftforge.registries.DataPackRegistriesHooks.getDataPackRegistries());
    }
    ...
    private <T> Optional<RegistryOps.RegistryInfo<T>> lookup(
        ...
        return ((Optional) fallback.lookup((ResourceKey) key))
                .map(value -> new RegistryOps.RegistryInfo<T>((HolderLookup.RegistryLookup<T>) value, (HolderLookup.RegistryLookup<T>) value, Lifecycle.stable()));
    }
    ...
    private <T> void registerAll(MappedRegistry<T> registry, JsonObject object, Codec<T> codec) {
        ...
        for (Map.Entry<String, JsonElement> entry : object.entrySet()) {
            ...
            try {
                T value = codec.parse(ops, entry.getValue()).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
