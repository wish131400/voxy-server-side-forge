    ...
    static BiomeSource decode(JsonObject section, long seed, ClientWorldgenRegistries registries) {
        ...
        try {
            ...
            for (var element : section.getAsJsonArray("slices")) {
                ...
                Object slice = codec.parse(registries.ops(), entry.get("slice")).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
                slices.add(Pair.of(new ResourceLocation(entry.get("name").getAsString()), slice));
            }
