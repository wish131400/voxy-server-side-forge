...
public final class PredictionMeshBuilder {
    ...
    private static void addEditedGround(VertexAccumulator out, ClientColumnSample[] samples, int[] heights,
        ...
        for (int z = 0; z < axis; z++) for (int x = 0; x < axis; x++) {
            ...
            int source = Math.max(0, Math.min(grid - 1, Math.floorDiv(pz, step))) * grid
                    + Math.max(0, Math.min(grid - 1, Math.floorDiv(px, step)));
    ...
    private static void addSimpleVegetation(VertexAccumulator out, PredictionSimpleVegetation.Form form, int tint) {
        ...
        if (form.tree() == null) {
            var state = net.minecraft.world.level.block.Blocks.GRASS.defaultBlockState();
