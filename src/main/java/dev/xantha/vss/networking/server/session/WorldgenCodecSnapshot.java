...
final class WorldgenCodecSnapshot {
    ...
    private static JsonObject encodeTemplates(net.minecraft.server.packs.resources.ResourceManager resources) {
        ...
        for (var entry : resources.listResources("structures", id -> id.getPath().endsWith(".nbt"))
            ...
            try (var input = entry.getValue().open()) {
                ...
                ResourceLocation id = entry.getKey().withPath(path.substring("structures/".length(), path.length() - 4));
    ...
    static Encoded encodeGenerator(ChunkGenerator generator, RegistryAccess access, DensityFunctionReferences references,
        ...
        if (noiseSeeds.size() != 0) root.add("noise_seed_aliases", noiseSeeds);
        ...
        for (var entry : access.registryOrThrow(Registries.DENSITY_FUNCTION).entrySet()) {
            try {
                var encoded = DensityFunction.DIRECT_CODEC.encodeStart(ops,
                        DensityFunctionSnapshot.applied(entry.getValue())).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
    ...
    static JsonElement encodeBiomeSource(BiomeSource source, RegistryAccess access,
        ...
        if (encoded.has("preset")) {
            ...
            JsonObject resolved = MultiNoiseBiomeSource.DIRECT_CODEC.codec()
                    .encodeStart(ops, preset.parameters()).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }).getAsJsonObject();
        ...
        if (encoded.has("biomes")
            ...
            HolderSet biomes = Biome.LIST_CODEC.parse(ops, encoded.get("biomes")).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
    ...
    private static <T> JsonObject encodeRegistry(
        ...
        for (Map.Entry<ResourceKey<T>, T> entry : access.registryOrThrow(registryKey).entrySet()) {
            JsonElement encoded;
            try {
                encoded = codec.encodeStart(ops, entry.getValue()).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
