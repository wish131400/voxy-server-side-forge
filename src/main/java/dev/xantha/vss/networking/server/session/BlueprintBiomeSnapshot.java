 ...
 * codecs ({@code ModdedBiomeSource.java:55-64}). A client that rebuilds the
 * generator from JSON alone therefore sees a plain forwarder, while the
 * running server routes a large share of the world through slices. This
 * snapshot serializes exactly the missing inputs.
 *
 ...
 * here is the order the client must replay. Registration order is preserved
 * by the caller's registry iteration.
 *
...
final class BlueprintBiomeSnapshot {
    ...
    private static JsonElement encode(DynamicOps<JsonElement> ops, Codec<?> codec, Object value) {
        return ((Codec<Object>) codec).encodeStart(ops, value)
