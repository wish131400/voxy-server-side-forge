...
class NbtSectionSerializerTest {
    ...
    @BeforeAll static void setup() throws Exception {
        if (net.minecraftforge.fml.loading.FMLPaths.GAMEDIR.get() == null) {
            net.minecraftforge.fml.loading.FMLPaths.loadAbsolutePaths(java.nio.file.Path.of("build", "tmp", "nbt-tests"));
        }
        net.minecraftforge.fml.loading.LoadingModList.of(List.of(), List.of(), null);
        // Reuse the repository's standalone-JUnit substitute for ModLauncher initialization.
        var prepare = Class.forName("dev.xantha.vss.client.prediction.ForgeTestBootstrap")
                .getDeclaredMethod("prepare");
        ...
        VanillaRegistries.createLookup().lookupOrThrow(Registries.BIOME).listElements()
                .forEach(h -> biomes.register(h.key(), h.value(), Lifecycle.stable()));
