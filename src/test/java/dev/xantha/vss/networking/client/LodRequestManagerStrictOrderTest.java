...
class LodRequestManagerStrictOrderTest {
    ...
    @BeforeAll static void initializeConfigDirectory() throws Exception {
        if (net.minecraftforge.fml.loading.FMLPaths.GAMEDIR.get() == null) {
            net.minecraftforge.fml.loading.FMLPaths.loadAbsolutePaths(java.nio.file.Files.createTempDirectory("vss-strict-test"));
        }
        Class<?> bootstrap = Class.forName("dev.xantha.vss.client.prediction.ClientTerrainSamplerTest");
        Method initialize = bootstrap.getDeclaredMethod("bootstrapMinecraft");
