...
/**
 * 1.20.1 counterpart of the neoforge memo benchmark. Measures what
 * {@link DensityMemo} buys the Java density march and asserts it never changes
 * a sampled height.
 *
...
class DensityMemoBenchTest {
    ...
    static void bootstrap() {
        ...
        lookup.lookupOrThrow(key).listElements().forEach(
                h -> biomes.register(h.key(), h.value(), com.mojang.serialization.Lifecycle.stable()));
    ...
    static ClientTerrainSampler sampler(JsonObject doc, long seed) {
        ...
        var biome = net.minecraft.world.level.biome.BiomeSource.CODEC
                .parse(ops, doc.get("biome_source"))
                .getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
    ...
    void memoPreservesHeightsAndItsCostIsMeasured() throws Exception {
        ...
        // Isolate density evaluation from repeated exact-height hits.
