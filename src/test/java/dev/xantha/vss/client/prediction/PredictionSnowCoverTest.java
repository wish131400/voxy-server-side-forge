...
class PredictionSnowCoverTest {
    ...
    void waterWarmGroundVoidAndUnsupportedSurfacesDoNotGetSnowTexture() {
        ...
        var unsupported = sample(90, 90, 0, ClientColumnSample.FLAG_SNOW, Blocks.GRASS);
        assertEquals(id(Blocks.GRASS), PredictionMeshBuilder.surfaceSpriteBlock(unsupported));
    ...
    void vanillaSurfaceRuleKeepsGrassButPublishesItsSnowCover() {
        ...
        vanilla.lookupOrThrow(Registries.BIOME).listElements().forEach(h -> biomes.register(h.key(), h.value(), Lifecycle.stable()));
        biomes.freeze();
