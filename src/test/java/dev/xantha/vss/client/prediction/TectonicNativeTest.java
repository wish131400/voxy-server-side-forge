        ...
        try(var loader=new URLClassLoader(new java.net.URL[]{url},getClass().getClassLoader())) {
            ...
            for(int i=0;i<names.size();i++) {
                ...
                doc.getAsJsonObject("noises").add(names.get(i).toString(),
                        net.minecraft.world.level.levelgen.synth.NormalNoise.NoiseParameters.DIRECT_CODEC.encodeStart(JsonOps.INSTANCE,parameters).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }));
                ...
                try {
                    ...
                    for(int i=0;i<names.size();i++) {
                        ...
                        var parameters=net.minecraft.world.level.levelgen.synth.NormalNoise.NoiseParameters.DIRECT_CODEC
                                .parse(JsonOps.INSTANCE,doc.getAsJsonObject("noises").get(names.get(i).toString())).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
    ...
    void releasedInvertCodecRunsInRustWithMatchingIeeeResults() throws Exception {
        ...
        try (var loader = new URLClassLoader(new java.net.URL[]{url}, getClass().getClassLoader())) {
            ...
            for (int i = 0; i < inputs.size(); i++) {
                ...
                var json = codec.encodeStart(JsonOps.INSTANCE, function).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }).getAsJsonObject();
