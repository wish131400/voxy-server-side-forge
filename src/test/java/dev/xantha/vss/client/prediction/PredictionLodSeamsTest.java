...
class PredictionLodSeamsTest {
    ...
    @Test void mixedLevelsConnectExactlyTheDisplayedHeightsOnBothAxesAndHeightOrders() {
        for (int step : new int[]{1, 2, 4, 16, 64}) for (boolean alongX : new boolean[]{true, false})
            for (boolean fineHigher : new boolean[]{true, false}) {
                ...
                var packed = patches.get(0).mesh();
                assertEquals(fine, patches.get(0).surface().tile());
    ...
    @Test void unrelatedUploadsReuseExistingSeamPayloads() {
        ...
        var cache = new PredictionLodSeams();
        var initial = cache.update(inputs).get(0).mesh();
        byte[] mask = cache.boundaryMask(inputs.get(0).tile().key());
        inputs.add(surface(tile(100, 100, 16, 80)));
        assertSame(initial, cache.update(inputs).get(0).mesh());
        assertSame(mask, cache.boundaryMask(inputs.get(0).tile().key()), "unrelated uploads do not rebuild boundary masks");
        cache.clear();
        assertNotSame(initial, cache.update(inputs).get(0).mesh());
    }
    ...
    @Test void copiedCoverageCanonicalizesNeighborsButChangedCoverageInvalidatesThem() {
        ...
        var initial = seams.update(List.of(fine, coarse)).get(0).mesh();
        var copy = new PredictionLodSeams.Surface(coarse.tile(), coarse.allowed().clone());
        var distant = surface(tile(100, 100, 16, 80));
        assertSame(initial, seams.update(List.of(fine, copy, distant)).get(0).mesh());
        ...
        assertEquals(area(initial), area(seams.update(List.of(fine, coarse, distant)).get(0).mesh()));
    }
    ...
    @Test void underwaterSeamsUseTheSameWaterDepthLightingAsOrdinaryWalls() {
        ...
        var mesh = new PredictionLodSeams().update(List.of(surface(fine), surface(coarse))).get(0).mesh();
        for (int q = 0; q < mesh.quadCount(); q++) {
            int i = q * 12; int[] w = mesh.quads();
            for (int corner = 0; corner < 4; corner++) {
                int yw = w[i + (corner < 2 ? 4 : 5)];
                int y = (((yw >>> ((corner & 1) * 16)) & 65535) - 32768) / 4;
                assertEquals(Math.max(0, Math.min(15, 55 - y)), w[i + (corner == 0 ? 7 : 8 + corner)] >>> 28,
