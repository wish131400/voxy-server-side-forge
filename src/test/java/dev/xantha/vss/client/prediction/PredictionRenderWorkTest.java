...
class PredictionRenderWorkTest {
    ...
    @Test void geometryFollowsCurrentCameraAndSnapshotReplacement() {
        ...
        assertSame(a,near.get(0).geometry().tile());
        var moved=cache.visible(new Vec3(96,120,96),null,8192);
        assertSame(b,moved.get(0).geometry().tile());
        ...
        assertSame(replacement,cache.visible(Vec3.ZERO,null,8192).get(0).geometry().tile());
    ...
    @Test void compareRepeatedGeometrySortWithFrameLocalDistances() {
        ...
        long[] allocation=new long[2];long thread=Thread.currentThread().getId();
