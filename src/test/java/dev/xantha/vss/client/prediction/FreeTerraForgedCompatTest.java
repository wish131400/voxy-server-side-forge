...
class FreeTerraForgedCompatTest {
    ...
    void releasedApiAndRuntimeOnlyPresetRoundTrip() throws Exception {
        {
            ...
            registry.register(id, preset, Lifecycle.stable());
            registry.freeze();
            ...
            assertEquals(codec.encodeStart(JsonOps.INSTANCE, preset).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }),
                    codec.encodeStart(JsonOps.INSTANCE, decoded).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }));
    ...
    private static void assertNoiseSnapshotSamples(ClassLoader loader, Object preset, Codec<Object> presetCodec,
        ...
        var entriesField = net.minecraftforge.registries.DeferredRegister.class.getDeclaredField("entries");
        ...
        for (var field : loader.loadClass(prefix + "registries.RTFBuiltInRegistries").getFields()) {
            ...
            var entries = (java.util.Map<net.minecraftforge.registries.RegistryObject<?>, java.util.function.Supplier<?>>)
        ...
        var bootstrap = new net.minecraft.data.worldgen.BootstapContext<Object>() {
            @Override public Holder.Reference<Object> register(ResourceKey<Object> key, Object value, Lifecycle lifecycle) {
                return noises.register(key, value, Lifecycle.stable());
            }
        ...
        loader.loadClass(prefix + "data.worldgen.preset.PresetNoiseData").getMethod("bootstrap",
                preset.getClass(), net.minecraft.data.worldgen.BootstapContext.class).invoke(null, preset, bootstrap);
        noises.freeze();
        ...
        List<RegistryDataLoader.RegistryData<?>> codecs = List.of(
                new RegistryDataLoader.RegistryData<>(presetKey, presetCodec),
                new RegistryDataLoader.RegistryData<>(noiseKey, codec));
    ...
    private static void assertActualInitializerSamples(ClassLoader loader, RegistryAccess source,
        ...
        try {
            ...
            for (int i = 0; i < 16; i++) {
                ...
                double expected = functions.get(0).compute(point);
                assertTrue(Double.isFinite(expected));
                assertEquals(expected, functions.get(functions.size() - 1).compute(point), 0.0);
            }
            ...
            for (int i = 0; i < 8; i++) {
                ...
                Object tile = tiles.get(0);
                Object cell = tile.getClass().getMethod("lookup", int.class, int.class).invoke(tile, x, z);
                Object lookup = contexts.get(0).getClass().getField("lookup").get(contexts.get(0));
                ...
                assertEquals(expected, functions.get(0).compute(point), 0.0);
                assertEquals(expected, functions.get(functions.size() - 1).compute(point), 0.0);
            }
            Object cell = loader.loadClass(prefix + "cell.Cell").getConstructor().newInstance();
            var cellClass = cell.getClass();
            var water = new FreeTerraForgedWater(contexts.get(0));
            ...
            Object levels = contexts.get(0).getClass().getField("levels").get(contexts.get(0));
