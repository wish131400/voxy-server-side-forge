...
class EpicTerrainCompatTest {
    ...
    void releasedPackSelectsReproducibleBackendAndPreservesExtendedHeight(String jar) throws Exception {
        ...
        try (var zip = new ZipFile(jar)) {
            ...
            if (zip.getEntry("data/minecraft/dimension/overworld.json") != null) {
                ...
                generator.add("biome_source", BiomeSource.CODEC.encodeStart(net.minecraft.resources.RegistryOps.create(JsonOps.INSTANCE, vanilla),
                        MultiNoiseBiomeSource.createFromList(parameters)).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }));
            }
        ...
        var settings = NoiseGeneratorSettings.DIRECT_CODEC.parse(registries.ops(), generator.get("settings")).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
        ...
        for (long seed : new long[]{0, -918273645L, 123456789L}) {
            ...
            var directGenerator = new NoiseBasedChunkGenerator(
                    BiomeSource.CODEC.parse(registries.ops(), generator.get("biome_source")).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }), Holder.direct(settings));
    ...
    private static void checkSplineRegressions(long world, JsonObject doc, ClientWorldgenRegistries registries) {
        for (boolean ordered : new boolean[]{true, false}) {
            ...
            var expected = DensityFunction.DIRECT_CODEC.parse(registries.ops(),
                    doc.getAsJsonObject("settings").getAsJsonObject("noise_router").get(key)).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
            var xyz = direct(9 * 12);
    ...
    private static <T> void export(JsonObject root, String name, HolderLookup.Provider lookup,
        ...
        lookup.lookupOrThrow(key).listElements().forEach(h ->
                values.add(h.key().location().toString(), codec.encodeStart(ops, h.value()).getOrThrow(false, message -> { throw new IllegalArgumentException(message); })));
