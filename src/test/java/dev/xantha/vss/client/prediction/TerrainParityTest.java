...
class TerrainParityTest {
    ...
    private static void registerCodec(ClassLoader loader,String name,String id) throws Exception {
        ...
        if (!registry.containsKey(key)) {
            ...
            try { net.minecraft.core.Registry.register(registry,key,
                    (com.mojang.serialization.Codec)loader.loadClass("dev.worldgen.tectonic.worldgen.densityfunction."+name).getField("DATA_CODEC").get(null)); }
    ...
    static void check(JsonObject doc, String name) throws Exception {
        ...
        var settings = NoiseGeneratorSettings.DIRECT_CODEC.parse(registries.ops(), doc.get("settings")).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
        ...
        compact.add("settings", references.settings(NoiseGeneratorSettings.DIRECT_CODEC.encodeStart(registries.ops(),
                DensityFunctionSnapshot.applied(settings)).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }).getAsJsonObject()));
