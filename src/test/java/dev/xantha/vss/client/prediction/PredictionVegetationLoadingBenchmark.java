...
class PredictionVegetationLoadingBenchmark {
    ...
    @Test void compareIdenticalNativeTilesWithAndWithoutLightVegetation() throws Exception {
        ...
        var source = BiomeSource.CODEC.parse(RegistryOps.create(JsonOps.INSTANCE, lookup), doc.get("biome_source")).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
        ...
        for (int step : new int[]{4, 8, 16}) for (int round = 0; round < 3; round++) {
            ...
            var profile = new DimensionProfile(new ResourceLocation("minecraft", "overworld"),seed,-64,384,"noise","minecraft:overworld",seed);
