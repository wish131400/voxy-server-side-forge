...
class DensityFunctionSnapshotTest {
    ...
    void nestedDirectHoldersRoundTripWithoutChangingDensity() {
        ...
        assertThrows(UnsupportedOperationException.class,
                () -> DensityFunction.DIRECT_CODEC.encodeStart(JsonOps.INSTANCE, graph).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }));
    ...
    void appliedNoiseSettingsPreserveSeededDensityAndRemainBounded() {
        ...
        var ops = net.minecraft.resources.RegistryOps.create(JsonOps.INSTANCE, lookup);
        var encoded = NoiseGeneratorSettings.DIRECT_CODEC.encodeStart(ops,
                DensityFunctionSnapshot.applied(original)).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
        assertTrue(encoded.toString().length() < 2_000_000, "expanded router must fit the network budget");
        var decoded = NoiseGeneratorSettings.DIRECT_CODEC.parse(ops, encoded).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
    ...
    void repeatedAppliedGraphUsesBoundedSharedRegistryReferences() {
        ...
        var expanded = DensityFunction.DIRECT_CODEC.encodeStart(JsonOps.INSTANCE, graph).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
        ...
        var decoded = DensityFunction.DIRECT_CODEC.parse(registries.ops(), compact).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
    ...
    void installedTectonicDefaultConfigFunctionsRoundTrip() throws Exception {
        ...
        try (var loader = new java.net.URLClassLoader(new java.net.URL[]{url}, getClass().getClassLoader())) {
            ...
            var encoded = DensityFunction.DIRECT_CODEC.encodeStart(JsonOps.INSTANCE,
                    DensityFunctionSnapshot.applied(configured)).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
            var restored = DensityFunction.DIRECT_CODEC.parse(JsonOps.INSTANCE, encoded).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }).mapAll(seedNoise);
            for (int i = -16; i <= 16; i++) {
    ...
    private static void assertRoundTrip(DensityFunction graph) {
        var encoded = DensityFunction.DIRECT_CODEC.encodeStart(JsonOps.INSTANCE,
                DensityFunctionSnapshot.applied(graph)).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
        var decoded = DensityFunction.DIRECT_CODEC.parse(JsonOps.INSTANCE, encoded).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
