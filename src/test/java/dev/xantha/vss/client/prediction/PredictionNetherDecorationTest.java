...
class PredictionNetherDecorationTest {
    ...
    @BeforeAll static void bootstrap() {
        ...
        lookup.lookupOrThrow(Registries.BIOME).listElements().forEach(h -> registry.register(h.key(), h.value(), Lifecycle.stable()));
    ...
    static com.google.gson.JsonObject nativeDocument(String biome) throws Exception {
        ...
        doc.add("block_definitions",RustWorldgenDocument.blockDefinitions());
    ...
    @Test void chunkFacingDecorationReadsAndWritesShareTransactions() {
        var terrain=sampler(Biomes.CRIMSON_FOREST,true);
        var level=new PredictionDecorationLevel(terrain,terrain,access,0,0);
        var chunk=level.getChunk(0,0,net.minecraft.world.level.chunk.ChunkStatus.FEATURES,true);
        ...
        assertThrows(UnsupportedOperationException.class,()->level.getChunk(3,0,net.minecraft.world.level.chunk.ChunkStatus.FEATURES,true));
    }
    ...
    @Test void nativeRegisteredNetherDecorationsUseTheInteriorStage() throws Exception {
        ...
        lookup.lookupOrThrow(Registries.PLACED_FEATURE).listElements().forEach(h->registry.register(h.key(),h.value(),Lifecycle.stable()));
