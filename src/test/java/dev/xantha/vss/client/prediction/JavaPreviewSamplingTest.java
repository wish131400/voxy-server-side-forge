...
class JavaPreviewSamplingTest {
    ...
    @BeforeAll static void bootstrap() {
        ...
        lookup.lookupOrThrow(key).listElements().forEach(h->biomes.register(h.key(),h.value(),com.mojang.serialization.Lifecycle.stable()));
    ...
    private static ClientTerrainSampler sampler(JsonObject doc, long seed) {
        var ops=net.minecraft.resources.RegistryOps.create(com.mojang.serialization.JsonOps.INSTANCE, lookup);
        var settings=net.minecraft.world.level.levelgen.NoiseGeneratorSettings.DIRECT_CODEC.parse(ops,doc.get("settings")).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
        var biome=net.minecraft.world.level.biome.BiomeSource.CODEC.parse(ops,doc.get("biome_source")).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
    ...
    @Test void approximateBiomeAndSurfaceContextsCannotLeakIntoExactColors() throws Exception {
        ...
        var biomes = new net.minecraft.world.level.biome.BiomeSource() {
            @Override protected com.mojang.serialization.Codec<? extends net.minecraft.world.level.biome.BiomeSource> codec() { throw new UnsupportedOperationException(); }
