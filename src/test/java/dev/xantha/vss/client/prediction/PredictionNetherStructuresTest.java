...
class PredictionNetherStructuresTest {
    ...
    @Test void actualFortressAndBastionPiecesSurviveOnJavaAndNativeTerrain() throws Exception {
        ...
        var biome=Biome.DIRECT_CODEC.encodeStart(ops,lookup.lookupOrThrow(Registries.BIOME).getOrThrow(Biomes.NETHER_WASTES).value()).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }).getAsJsonObject();
        ...
        lookup.lookupOrThrow(Registries.TEMPLATE_POOL).listElements().filter(h->h.key().location().getPath().startsWith("bastion/")||h.key().location().getPath().equals("empty"))
                .forEach(h->pools.add(h.key().location().toString(),net.minecraft.world.level.levelgen.structure.pools.StructureTemplatePool.DIRECT_CODEC.encodeStart(ops,h.value()).getOrThrow(false, message -> { throw new IllegalArgumentException(message); })));
        ...
        lookup.lookupOrThrow(Registries.PROCESSOR_LIST).listElements().filter(h->pools.toString().contains("\""+h.key().location()+"\"")).forEach(h->processors.add(h.key().location().toString(),
                net.minecraft.world.level.levelgen.structure.templatesystem.StructureProcessorType.DIRECT_CODEC.encodeStart(ops,h.value()).getOrThrow(false, message -> { throw new IllegalArgumentException(message); })));
        ...
        for(var structureKey:List.of(BuiltinStructures.FORTRESS,BuiltinStructures.BASTION_REMNANT)) {
            ...
            var structure=Structure.DIRECT_CODEC.encodeStart(ops,lookup.lookupOrThrow(Registries.STRUCTURE).getOrThrow(structureKey).value()).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }).getAsJsonObject();
    ...
    private static JsonObject loadBastionTemplates() throws Exception {
        ...
        var probe=PredictionNetherStructuresTest.class.getResource("/data/minecraft/structures/village/plains/houses/plains_small_house_1.nbt");
        assertNotNull(probe);
        if(probe.openConnection() instanceof java.net.JarURLConnection connection) {
            ...
            for(var entry:Collections.list(jar.entries())) {
                String prefix="data/minecraft/structures/";
