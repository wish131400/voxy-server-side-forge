...
class WorldgenRegistryDependenciesTest {
    ...
    @Test void dependencyClosureRestoresUnsyncedRegistriesAndForwardHolders() {
        ...
        var holder = config.register(configId, 731, Lifecycle.stable());
        config.freeze();
        ...
        var regionHolder = dependent.register(regionId, holder, Lifecycle.stable());
        dependent.freeze();
        ...
        List<RegistryDataLoader.RegistryData<?>> codecs = List.of(
                new RegistryDataLoader.RegistryData<>(dependentKey, holderCodec),
                new RegistryDataLoader.RegistryData<>(configKey, Codec.INT),
                new RegistryDataLoader.RegistryData<>(unusedKey, Codec.INT));
        ...
        var encoded = regionCodec.encodeStart(dependencies.ops(), regionHolder).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
        ...
        assertEquals(731, regionCodec.parse(client.ops(), encoded).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }).value().value());
            ...
            for (var entry : archive.stream().filter(e -> e.getName().startsWith(prefix)
                ...
                try (var reader = new java.io.InputStreamReader(archive.getInputStream(entry), java.nio.charset.StandardCharsets.UTF_8)) {
                    ...
                    registry.register(ResourceKey.create(key, id),
                            codec.parse(com.mojang.serialization.JsonOps.INSTANCE, json).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }), Lifecycle.stable());
                }
            }
            registry.freeze();
            assertTrue(registry.size() > 5);
            List<RegistryDataLoader.RegistryData<?>> codecs = List.of(new RegistryDataLoader.RegistryData<>(key, codec));
    ...
    @Test void emptyRuntimeRegistryIsPreservedAndAbsentModIsNotRequired() {
        ...
        registry.freeze();
        List<RegistryDataLoader.RegistryData<?>> codecs = List.of(new RegistryDataLoader.RegistryData<>(key, Codec.INT));
        var server = new RegistryAccess.ImmutableRegistryAccess(List.of(registry));
        var dependencies = new WorldgenRegistryDependencies(server, codecs);
        var client = ClientWorldgenRegistries.decode(root(dependencies.encode()), RegistryAccess.EMPTY, codecs);
        assertEquals(0, client.access().registryOrThrow(key).size());
        assertTrue(new WorldgenRegistryDependencies(RegistryAccess.EMPTY, codecs).encode().size() == 0);
    }
        ...
        try (var loader = new java.net.URLClassLoader(urls.toArray(java.net.URL[]::new), getClass().getClassLoader())) {
            ...
            for (String dimension : List.of("overworld", "the_nether", "the_end")) {
                ...
                try (var input = loader.getResourceAsStream(path)) {
                    ...
                    var value = codec.parse(com.mojang.serialization.JsonOps.INSTANCE, json).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
                    configType.getMethod("bind", long.class).invoke(value, seed);
                    registry.register(ResourceKey.create(key, ResourceLocation.parse("lithostitched:region/" + dimension)), value, Lifecycle.stable());
                }
            }
            registry.freeze();
            ...
            var encoded = densityCodec.encodeStart(dependencies.ops(), expected).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
            var root = root(dependencies.encode());
            var client = ClientWorldgenRegistries.decode(root, RegistryAccess.EMPTY, codecs);
            var actual = densityCodec.parse(client.ops(), encoded).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
            ...
            assertNotEquals(actual.compute(point), densityCodec.parse(second.ops(), encoded).getOrThrow(false, message -> { throw new IllegalArgumentException(message); }).compute(point));
        }
    }
