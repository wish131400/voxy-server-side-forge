...
class RustVegetationStageTest {
    ...
    void reusesVolumeAcrossStepsAndUploadsInterveningJavaEdits() throws Exception {
        ...
        var doc = JsonParser.parseString(LithostitchedNativeTest.document().toString()
        ...
        var first = JsonParser.parseString("""
                {"feature":{"type":"minecraft:simple_block","config":{"to_place":{
                  "type":"minecraft:simple_state_provider","state":{"Name":"minecraft:grass"}}}},
        ...
        doc.add("input_states", JsonParser.parseString("""
                [{"Name":"minecraft:air"},{"Name":"minecraft:grass"},
                 {"Name":"minecraft:dirt"},{"Name":"minecraft:dandelion"}]
                """));
        var ops = net.minecraft.resources.RegistryOps.create(JsonOps.INSTANCE, net.minecraft.data.registries.VanillaRegistries.createLookup());
        var firstFeature = PlacedFeature.DIRECT_CODEC.parse(ops,first).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
        var secondFeature = PlacedFeature.DIRECT_CODEC.parse(ops,second).getOrThrow(false, message -> { throw new IllegalArgumentException(message); });
        ...
        try (var sampler = new RustTerrainSampler(world,profile,context)) {
            ...
            try (stage) {
                ...
                assertEquals(Blocks.GRASS.defaultBlockState(),level.placed().get(new BlockPos(0,targetY,0)));
