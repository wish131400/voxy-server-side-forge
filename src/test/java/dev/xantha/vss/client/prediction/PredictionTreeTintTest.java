...
class PredictionTreeTintTest {
    ...
    @Test void vanillaSignedLeafColoursAreValidTints() throws Exception {
        ...
        assertTrue(FoliageColor.getEvergreenColor() > 0);
        assertTrue(FoliageColor.getBirchColor() > 0);
        assertEquals(0xFF619961, tints[id(Blocks.SPRUCE_LEAVES)]);
        assertEquals(0xFF80A755, tints[id(Blocks.BIRCH_LEAVES)]);
        assertEquals(0xFF000000 | FoliageColor.getDefaultColor(), tints[id(Blocks.OAK_LEAVES)]);
    ...
    @Test void unavailableProviderDoesNotEraseOtherTintsOrRejectBlack() {
        ...
        assertEquals(0xFF000000 | FoliageColor.getEvergreenColor(), tints[id(Blocks.SPRUCE_LEAVES)]);
    ...
    private static BlockColors vanillaColors() throws Exception {
        ...
        var previous = net.minecraftforge.fml.ModList.get();
        try {
            var empty = net.minecraftforge.fml.ModList.of(java.util.List.of(), java.util.List.of());
            var loaded = net.minecraftforge.fml.ModList.class.getDeclaredMethod("setLoadedMods", java.util.List.class);
            loaded.setAccessible(true);
            loaded.invoke(empty, java.util.List.of());
            return BlockColors.createDefault();
        } finally {
            field(net.minecraftforge.fml.ModList.class, "INSTANCE").set(null, previous);
        }
    }
    ...
    private int installTexture(Block block) throws Exception {
        ...
        try (var input = getClass().getResourceAsStream("/assets/minecraft/textures/block/" + name + ".png")) {
            ...
            try (var contents = new SpriteContents(ResourceLocation.withDefaultNamespace("block/" + name),
                    new FrameSize(16, 16), NativeImage.read(input), net.minecraft.client.resources.metadata.animation.AnimationMetadataSection.EMPTY)) {
